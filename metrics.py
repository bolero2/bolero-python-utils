...
def get_metric_for_detection2(
        ...
        if dt_label == '' and dt_bbox == [] and dt_conf == -1:
            ...
            for gt_label, gt_bbox in zip(gt_classes, gt_bboxes):
                ...
                if (iou_val >= iou_threshold):# and (dt_label == gt_label) and ([imgpath, gt_bbox, gt_label] not in already_founded_gt):
                    ...
                    if (dt_label == gt_label):# and ([imgpath, gt_bbox, gt_label] not in already_founded_gt):
                        accurate_count += 1
                        dt_count[dt_label] += 1
    ...
    print("Found box:", found_box)
    print("Accurate Count:", accurate_count)
    print("Rate : ", round(accurate_count / found_box, 4))

    print("============ Ground-Truth Count per Labels ============ ")
    for k, v in gt_count.items():
        print(f" -> {k} : {v}")

    print("============ Detected Count per Labels ============ ")
    for k, v in dt_count.items():
        print(f" -> {k} : {v}")
    ...
    print()
    for __l in __labels:
        print(f" -> {__l} accuracy :", round(dt_count[__l] / gt_count[__l], 4))

    print("\n -> Total Accuracy :", round(sum(dt_count.values()) / sum(gt_count.values()), 4))
    print()
    exit(0)
    # print("LEN already_founded_gt :", len(already_founded_gt))
    TPFP_columns = ['input_data', 'predicted_y_classname', 'predicted_y_bndboxes', 'confidence', 'avg_iou', 'tp', 'fp']
...
def get_metric_for_segmentation(dataframe, csv_path, category_list=1, rootpath=[]):
    # yt_rootpath = path_join(rootpath[0], rootpath[1][0])
    # yp_rootpath = path_join(rootpath[0], rootpath[1][1])
    ...
    for elem in tqdm(df_list, desc=" Comparing between GT and prediction... "):
        ...
        gt_image = Image.open(os.path.join(yt_rootpath, gt))
        gt_image = np.array(gt_image)
        target_image = Image.open(os.path.join(yp_rootpath, target))
