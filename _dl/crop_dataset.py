...
if __name__ == "__main__":
    ...
    for ti, tr in zip(test_images, test_result):
        ...
        for r in tr:
            ...
            print(np.array(cropped_annot), cropped_annot.size, np.unique(cropped_annot))

            if sum_uniq != 0:
                saved_imagepath = glob(os.path.join(savedir, "images"))

                basename = basename + "_" + str(len(os.listdir(os.path.join(savedir, "images"))))
                print("Savename :", basename)

                cv2.imshow("image", cv2.resize(cropped_image, (640, 480)))
                cv2.imshow("annot", cv2.resize(np.array(cropped_annot.convert("RGB")), (640, 480)))
...
if __name__ == "__main__":
    ...
    pkl_file = os.path.join(agent.save_dir, f"test_{agent.hash}.pkl")
    ...
    get_metric_for_detection(result_df, csv_path=os.path.join(agent.save_dir, "test-result.csv"), category_list=['dough'])
"""
